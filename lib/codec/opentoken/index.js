...
class OpenToken {
	...
	static CIPHER_DEFAULT = OpenToken.CIPHER_AES_256_CBC;
	...
	encode(payload, cipherId = OpenToken.CIPHER_DEFAULT) {
		if (!payload) {
			throw new Error('Invalid Token.');
		}
	...
	_hmac(data) {
		...
		const hmac = createHmac(OpenToken.CIPHER_HMAC_HASH, Buffer.from(this._generateKey(cipherId), 'base64'));
		return hmac
	...
	_generateKey(cipherId = OpenToken.CIPHER_DEFAULT, salt = Buffer.from([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])) {
		const keyLength = OpenTokenCipher.keyLength(cipherId) / 8;
		const iterations = 1000;
		return pbkdf2Sync(this.password, salt, iterations, keyLength, OpenToken.CIPHER_HMAC_HASH).toString('base64');
	}
	...
	_encode(payload, cipherId = OpenToken.CIPHER_DEFAULT) {
	...
	_encryptPayload(payload, iv, cipherId) {
		...
		const algorithm = OpenTokenCipher.algorithm(cipherId);
		const cipher = createCipheriv(algorithm, Buffer.from(this._generateKey(), 'base64'), iv);
